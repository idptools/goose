"""
Comprehensive pytest tests for create.sequence generation functions.
...
class TestSequenceGeneration:
    ...
    def test_shortest_sequence(self):
        """Test basic sequence generation with default parameters."""
        seq = create.sequence(parameters.MINIMUM_LENGTH)
        assert isinstance(seq, str)
        assert len(seq) == parameters.MINIMUM_LENGTH
        assert all(aa in 'ACDEFGHIKLMNPQRSTVWY' for aa in seq)

    def test_longest_sequence(self):
        """Test basic sequence generation with default parameters."""
        seq = create.sequence(parameters.MAXIMUM_LENGTH)
        assert isinstance(seq, str)
        assert len(seq) == parameters.MAXIMUM_LENGTH
        assert all(aa in 'ACDEFGHIKLMNPQRSTVWY' for aa in seq)

    def test_sequence_with_fcr(self):
        """Test sequence generation with FCR parameter."""
        seq = create.sequence(50, FCR=0.3)
        assert isinstance(seq, str)
        assert len(seq) == 50
        fcr_actual = Protein(seq).FCR
        # Allow some tolerance for FCR matching
        assert abs(fcr_actual - 0.3) < 0.001

    def test_sequence_with_ncpr(self):
        """Test sequence generation with NCPR parameter."""
        seq = create.sequence(50, NCPR=0.2)
        assert isinstance(seq, str)
        assert len(seq) == 50
        ncpr_actual = Protein(seq).NCPR
        # Allow some tolerance for NCPR matching
        assert abs(ncpr_actual - 0.2) < 0.001

    def test_sequence_with_hydropathy(self):
        """Test sequence generation with hydropathy parameter."""
        seq = create.sequence(50, hydropathy=3.0)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert abs(Protein(seq).hydrophobicity - 3.0) < parameters.MAXIMUM_HYDRO_ERROR

    def test_sequence_with_kappa(self):
        """Test sequence generation with kappa parameter."""
        seq = create.sequence(50, kappa=0.5)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert abs(Protein(seq).kappa - 0.5) < parameters.MAXIMUM_KAPPA_ERROR

    def test_sequence_with_multiple_properties(self):
        """Test sequence generation with multiple properties."""
        seq = create.sequence(50, FCR=0.3, NCPR=0.1, hydropathy=3.0)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert abs(Protein(seq).FCR - 0.3) < 0.001
        assert abs(Protein(seq).NCPR - 0.1) < 0.001
        assert abs(Protein(seq).hydrophobicity - 3.0) < parameters.MAXIMUM_HYDRO_ERROR

    def test_sequence_with_all_properties(self):
        """Test sequence generation with multiple properties."""
        seq = create.sequence(50, FCR=0.3, NCPR=0.1, hydropathy=3.0, kappa=0.5)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert abs(Protein(seq).FCR - 0.3) < 0.001
        assert abs(Protein(seq).NCPR - 0.1) < 0.001
    ...
    def test_sequence_with_exclude_residues(self):
        ...
        seq = create.sequence(50, exclude=['C', 'M'])
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert 'C' not in seq
        assert 'M' not in seq
    ...
    def test_sequence_legacy_parameter_names(self):
        ...
        seq = create.sequence(50, hydrophobicity=3.0)
        assert isinstance(seq, str)
        assert len(seq) == 50

        # Test cutoff -> disorder_cutoff
        seq = create.sequence(50, cutoff=0.4)
        assert isinstance(seq, str)
        assert len(seq) == 50
    ...
    def test_sequence_invalid_parameters(self):
        ...
        # Test invalid FCR
        with pytest.raises(goose_exceptions.GooseInputError):
            create.sequence(50, FCR=1.5)
        
        # Test invalid NCPR
        with pytest.raises(goose_exceptions.GooseInputError):
            create.sequence(50, NCPR=2.0)
        
        # Test invalid hydropathy
        with pytest.raises(goose_exceptions.GooseInputError):
            create.sequence(50, hydropathy=10.0)
        
        # Test invalid kappa
        with pytest.raises(goose_exceptions.GooseInputError):
            create.sequence(50, kappa=2.0)

    def test_sequence_unknown_parameters(self):
        """Test sequence generation with unknown parameters."""
        with pytest.raises(goose_exceptions.GooseInputError):
            create.sequence(50, unknown_param=0.5)

    def test_sequence_extreme_parameters(self):
        """Test sequence generation with extreme but valid parameters."""
        # Test extreme FCR
        seq = create.sequence(50, FCR=1.0)
        assert isinstance(seq, str)
        assert len(seq) == 50
        
        # Test extreme NCPR
        seq = create.sequence(50, NCPR=-1.0)
        assert isinstance(seq, str)
        assert len(seq) == 50
...
class TestSeqByFractions:
    ...
    def test_basic_seq_by_fractions(self):
        ...
        seq = create.seq_by_fractions(50)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert all(aa in 'ACDEFGHIKLMNPQRSTVWY' for aa in seq)
    ...
    def test_seq_by_fractions_single_amino_acid(self):
        ...
        seq = create.seq_by_fractions(100, A=0.3)
        assert isinstance(seq, str)
        assert len(seq) == 100
    ...
    def test_seq_by_fractions_multiple_amino_acids(self):
        ...
        assert abs(a_fraction - 0.2) < 0.001
        assert abs(g_fraction - 0.1) < 0.001
        assert abs(s_fraction - 0.15) < 0.001
    ...
    def test_seq_by_fractions_all_amino_acids(self):
        ...
        seq = create.seq_by_fractions(100, **fractions)
        assert isinstance(seq, str)
        assert len(seq) == 100
        for aa in fractions.keys():
            assert (abs(seq.count(aa) / len(seq) - fractions[aa]) < 0.001)
    ...
    def test_seq_by_fractions_with_max_aa_fractions(self):
        ...
        seq = create.seq_by_fractions(100, A=0.4, max_aa_fractions=max_fractions)
        assert isinstance(seq, str)
        assert len(seq) == 100
    ...
    def test_seq_by_fractions_with_remaining_probabilities_dict(self):
        ...
        seq = create.seq_by_fractions(50, A=0.2, remaining_probabilities=remaining_probs)
        assert isinstance(seq, str)
        assert len(seq) == 50
    ...
    def test_seq_by_fractions_with_remaining_probabilities_string(self):
        ...
        seq = create.seq_by_fractions(50, A=0.2, remaining_probabilities='human')
        assert isinstance(seq, str)
        assert len(seq) == 50

    def test_seq_by_fractions_with_attempts(self):
        """Test sequence generation with different attempt numbers."""
        seq = create.seq_by_fractions(50, A=0.2, attempts=5)
        assert isinstance(seq, str)
        assert len(seq) == 50

    def test_seq_by_fractions_with_disorder_cutoff(self):
        """Test sequence generation with custom disorder cutoff."""
        seq = create.seq_by_fractions(50, A=0.2, disorder_cutoff=0.3)
        assert isinstance(seq, str)
        assert len(seq) == 50

    def test_seq_by_fractions_with_strict_disorder(self):
        """Test sequence generation with strict disorder checking."""
        seq = create.seq_by_fractions(50, A=0.2, strict_disorder=True)
        assert isinstance(seq, str)
        assert len(seq) == 50

    def test_seq_by_fractions_with_return_all_sequences(self):
        """Test sequence generation returning all sequences."""
        seqs = create.seq_by_fractions(30, A=0.2, return_all_sequences=True, attempts=3)
        assert isinstance(seqs, list)
        assert len(seqs) >= 1
        for seq in seqs:
            assert isinstance(seq, str)
    ...
    def test_seq_by_fractions_with_metapredict_version(self):
        """Test sequence generation with different MetaPredict versions."""
        for version in [1, 2, 3]:
            seq = create.seq_by_fractions(50, A=0.2, metapredict_version=version)
            assert isinstance(seq, str)
            assert len(seq) == 50

    def test_seq_by_fractions_with_ordered_constraints(self):
        """Test sequence generation with ordered residue constraints."""
        seq = create.seq_by_fractions(50, A=0.2, max_consecutive_ordered=5, 
    ...
    def test_seq_by_fractions_with_batch_size(self):
        """Test sequence generation with custom batch size."""
        seq = create.seq_by_fractions(50, A=0.2, batch_size=10)
        assert isinstance(seq, str)
        assert len(seq) == 50
    ...
    def test_seq_by_fractions_legacy_parameter_names(self):
        """Test sequence generation with legacy parameter names."""
        # Test cutoff -> disorder_cutoff
        seq = create.seq_by_fractions(50, A=0.2, cutoff=0.4)
        assert isinstance(seq, str)
        assert len(seq) == 50
    ...
    def test_seq_by_fractions_extreme_fractions(self):
        ...
        seq = create.seq_by_fractions(50, A=0.8)
        assert isinstance(seq, str)
        assert len(seq) == 50
        
        # Test very low fraction
        seq = create.seq_by_fractions(100, A=0.01)
        assert isinstance(seq, str)
        assert len(seq) == 100
...
class TestSeqByClasses:
    ...
    def test_seq_by_classes_single_class(self):
        ...
        seq =  create.seq_by_classes(100, aromatic=0.2)
        assert isinstance(seq, str)
        assert len(seq) == 100
        ...
        # Allow some tolerance for fraction matching
        assert abs(aromatic_fraction - 0.2) < 0.001

    def test_seq_by_classes_multiple_classes(self):
        """Test sequence generation with multiple class fractions."""
        seq =  create.seq_by_classes(100, aromatic=0.15, aliphatic=0.2, polar=0.1)
        assert isinstance(seq, str)
        assert len(seq) == 100
        
        # Check individual class fractions
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
        aliphatic_count = sum(1 for aa in seq if aa in 'AILVM')
        ...
        assert abs(aromatic_fraction - 0.15) < 0.001
        assert abs(aliphatic_fraction - 0.2) < 0.001
        assert abs(polar_fraction - 0.1) < 0.001

    def test_seq_by_classes_all_classes(self):
        """Test sequence generation with all class fractions specified."""
        seq =  create.seq_by_classes(100, aromatic=0.1, aliphatic=0.1, polar=0.2,
                                 positive=0.0, negative=0.2, glycine=0.15,
                                 proline=0.15, cysteine=0.05, histidine=0.05)
        assert isinstance(seq, str)
        assert len(seq) == 100
        # Check individual class fractions
        aromatic_fraction = seq.count('F') / len(seq) + seq.count('W') / len(seq) + seq.count('Y') / len(seq)
        aliphatic_fraction = seq.count('A') / len(seq) + seq.count('I') / len(seq) + seq.count('L') / len(seq) + seq.count('V') / len(seq) + seq.count('M') / len(seq)
        polar_fraction = seq.count('N') / len(seq) + seq.count('Q') / len(seq) + seq.count('S') / len(seq) + seq.count('T') / len(seq)
        positive_fraction = seq.count('K') / len(seq) + seq.count('R') / len(seq)
        negative_fraction = seq.count('D') / len(seq) + seq.count('E') / len(seq)
        glycine_fraction = seq.count('G') / len(seq)
        proline_fraction = seq.count('P') / len(seq)
        cysteine_fraction = seq.count('C') / len(seq)
        histidine_fraction = seq.count('H') / len(seq)

        assert abs(aromatic_fraction - 0.1) < 0.001
        assert abs(aliphatic_fraction - 0.1) < 0.001
        assert abs(polar_fraction - 0.2) < 0.001
        assert abs(positive_fraction - 0.0) < 0.001
        assert abs(negative_fraction - 0.2) < 0.001
        assert abs(glycine_fraction - 0.15) < 0.001
        assert abs(proline_fraction - 0.15) < 0.001
        assert abs(cysteine_fraction - 0.05) < 0.001
        assert abs(histidine_fraction - 0.05) < 0.001


    def test_seq_by_classes_charged_residues(self):
        """Test sequence generation with charged residue classes."""
        seq =  create.seq_by_classes(100, positive=0.2, negative=0.15)
        assert isinstance(seq, str)
        assert len(seq) == 100
        ...
        assert abs(positive_fraction - 0.2) < 0.001
        assert abs(negative_fraction - 0.15) < 0.001

    def test_seq_by_classes_special_residues(self):
        """Test sequence generation with special residue classes."""
        seq =  create.seq_by_classes(100, glycine=0.15, proline=0.1, 
        ...
        assert abs(g_fraction - 0.15) < 0.001
        assert abs(p_fraction - 0.1) < 0.001
        assert abs(c_fraction - 0.05) < 0.001
        assert abs(h_fraction - 0.05) < 0.001
    ...
    def test_seq_by_classes_with_max_class_fractions(self):
        ...
        seq =  create.seq_by_classes(100, aromatic=0.3, aliphatic=0.25, 
                                 max_class_fractions=max_fractions)
        assert isinstance(seq, str)
        assert len(seq) == 100
        # check fractions are what was specified
        aromatic_fraction = seq.count('F') / len(seq) + seq.count('W') / len(seq) + seq.count('Y') / len(seq)
        aliphatic_fraction = seq.count('A') / len(seq) + seq.count('I') / len(seq) + seq.count('L') / len(seq) + seq.count('V') / len(seq) + seq.count('M') / len(seq)

        assert abs(aromatic_fraction - 0.3) < 0.001
        assert abs(aliphatic_fraction - 0.25) < 0.001

    def test_seq_by_classes_with_remaining_probabilities_dict(self):
        """Test sequence generation with remaining probabilities dictionary."""
        remaining_probs = {'A': 0.5, 'G': 0.3, 'S': 0.2}
        seq =  create.seq_by_classes(50, aromatic=0.2, remaining_probabilities=remaining_probs)
        assert isinstance(seq, str)
        assert len(seq) == 50
    ...
    def test_seq_by_classes_with_remaining_probabilities_string(self):
        """Test sequence generation with remaining probabilities string."""
        seq =  create.seq_by_classes(50, aromatic=0.2, remaining_probabilities='human')
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
    ...
    def test_seq_by_classes_with_attempts(self):
        """Test sequence generation with different attempt numbers."""
        seq =  create.seq_by_classes(50, aromatic=0.2, num_attempts=15)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
    ...
    def test_seq_by_classes_with_disorder_cutoff(self):
        """Test sequence generation with custom disorder cutoff."""
        seq =  create.seq_by_classes(50, aromatic=0.2, disorder_cutoff=0.3)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
    ...
    def test_seq_by_classes_with_strict_disorder(self):
        """Test sequence generation with strict disorder checking."""
        seq =  create.seq_by_classes(50, aromatic=0.2, strict_disorder=True)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
    ...
    def test_seq_by_classes_with_metapredict_version(self):
        """Test sequence generation with different MetaPredict versions."""
        for version in [1, 2, 3]:
            seq =  create.seq_by_classes(50, positive=0.2, metapredict_version=version)
            assert isinstance(seq, str)
            assert len(seq) == 50
            # check aromatic fraction
            positive_count = sum(1 for aa in seq if aa in 'KR')
    ...
    def test_seq_by_classes_with_ordered_constraints(self):
        """Test sequence generation with ordered residue constraints."""
        seq =  create.seq_by_classes(50, aromatic=0.2, max_consecutive_ordered=5, 
                                 max_total_ordered=0.1)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
    ...
    def test_seq_by_classes_legacy_parameter_names(self):
        """Test sequence generation with legacy parameter names."""
        # Test cutoff -> disorder_cutoff
        seq =  create.seq_by_classes(50, aromatic=0.2, cutoff=0.4)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
    ...
    def test_seq_by_classes_length_validation(self):
        ...
        # Test minimum length
        with pytest.raises(goose_exceptions.GooseInputError):
             create.seq_by_classes(5, aromatic=0.2)

    def test_seq_by_classes_invalid_fractions(self):
        """Test sequence generation with invalid class fractions."""
        # Test fraction > 1
        with pytest.raises(goose_exceptions.GooseInputError):
             create.seq_by_classes(50, aromatic=1.5)
        
        # Test negative fraction
        with pytest.raises(goose_exceptions.GooseInputError):
             create.seq_by_classes(50, aromatic=-0.1)
        
        # Test sum of fractions > 1
        with pytest.raises(goose_exceptions.GooseInputError):
             create.seq_by_classes(50, aromatic=0.6, aliphatic=0.6)

    def test_seq_by_classes_extreme_fractions(self):
        """Test sequence generation with extreme but valid fractions."""
        # Test very high fraction
        seq =  create.seq_by_classes(50, aromatic=0.8, num_attempts=1000)
        assert isinstance(seq, str)
        assert len(seq) == 50
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
        aromatic_fraction = aromatic_count / len(seq)
        assert abs(aromatic_fraction - 0.8) < 0.001            
        
        # Test very low fraction
        seq =  create.seq_by_classes(100, aromatic=0.01)
        assert isinstance(seq, str)
        assert len(seq) == 100
        # check aromatic fraction
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
        aromatic_fraction = aromatic_count / len(seq)
        assert abs(aromatic_fraction - 0.01) < 0.001            
...
class TestSequenceGenerationIntegration:
    ...
    def test_seq_by_classes_consistency(self):
        ...
        # Multiple calls should produce valid sequences
        for _ in range(5):
            seq =  create.seq_by_classes(50, aromatic=0.2, aliphatic=0.1)
            assert isinstance(seq, str)
            assert len(seq) == 50
            aromatic_count = sum(1 for aa in seq if aa in 'FWY')
            aliphatic_count = sum(1 for aa in seq if aa in 'AILVM')
    ...
    def test_function_parameter_compatibility(self):
        ...
        # Test with sequence()
        seq1 = create.sequence(50, **common_params)
        assert isinstance(seq1, str)
        assert len(seq1) == 50
        
        # Test with seq_by_fractions()
        seq2 = create.seq_by_fractions(50, A=0.2, **common_params)
        assert isinstance(seq2, str)
        assert len(seq2) == 50
        assert abs(seq2.count('A') / len(seq2) - 0.2) < 0.001

        # Test with seq_by_classes()
        seq3 =  create.seq_by_classes(50, aromatic=0.2, **common_params)
        assert isinstance(seq3, str)
        assert len(seq3) == 50
        assert abs(seq3.count('F') / len(seq3) + seq3.count('W') / len(seq3) + seq3.count('Y') / len(seq3) - 0.2) < 0.001

    def test_edge_case_lengths(self):
        """Test sequence generation with edge case lengths."""
        # Test minimum allowed length
        seq = create.sequence(10)
        assert len(seq) == 10
        
        # Test larger lengths
        seq = create.sequence(200)
        assert len(seq) == 200
    ...
    def test_amino_acid_composition_validation(self):
        ...
        # Test sequence()
        seq = create.sequence(100, FCR=0.3, NCPR=0.1)
        ...
        seq = create.seq_by_fractions(100, A=0.2, G=0.1, S=0.1)
        assert set(seq).issubset(valid_aas)
        assert len(seq) == 100
        a_fraction = seq.count('A') / len(seq)
        g_fraction = seq.count('G') / len(seq)
        ...
        assert abs(a_fraction - 0.2) < 0.001
        assert abs(g_fraction - 0.1) < 0.001
        assert abs(s_fraction - 0.1) < 0.001
        
        # Test seq_by_classes()
        seq =  create.seq_by_classes(100, aromatic=0.2, aliphatic=0.1)
        assert set(seq).issubset(valid_aas)
        assert len(seq) == 100
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
        aliphatic_count = sum(1 for aa in seq if aa in 'AILVM')
    ...
    def test_performance_with_difficult_constraints(self):
        ...
        seq = create.sequence(50, FCR=0.4, NCPR=0.2, hydropathy=3.0, kappa=0.5, attempts=500)
        assert isinstance(seq, str)
        assert len(seq) == 50
        assert abs(Protein(seq).FCR - 0.4) < 0.001
        assert abs(Protein(seq).NCPR - 0.2) < 0.001
        assert abs(Protein(seq).hydrophobicity - 3.0) < parameters.MAXIMUM_HYDRO_ERROR
        assert abs(Protein(seq).kappa - 0.5) < parameters.MAXIMUM_KAPPA_ERROR
        
        # Test seq_by_fractions with many constraints
        seq = create.seq_by_fractions(50, A=0.3, G=0.2, S=0.2, attempts=5)
        assert isinstance(seq, str)
        assert len(seq) == 50
        a_fraction = seq.count('A') / len(seq)
        g_fraction = seq.count('G') / len(seq)
        s_fraction = seq.count('S') / len(seq)
        assert abs(a_fraction - 0.3) < 0.001
        assert abs(g_fraction - 0.2) < 0.001
        assert abs(s_fraction - 0.2) < 0.001
        
        # Test seq_by_classes with many constraints
        seq =  create.seq_by_classes(50, aromatic=0.2, aliphatic=0.2, polar=0.2, 
                                 positive=0.1, num_attempts=5)
        assert isinstance(seq, str)
        assert len(seq) == 50
        aromatic_count = sum(1 for aa in seq if aa in 'FWY')
        aliphatic_count = sum(1 for aa in seq if aa in 'MAILV')
        polar_count = sum(1 for aa in seq if aa in 'NQST')
        positive_count = sum(1 for aa in seq if aa in 'KR')
        ...
        assert abs(aliphatic_fraction - 0.2) < 0.001
        assert abs(polar_fraction - 0.2) < 0.001
        assert abs(positive_fraction - 0.1) < 0.001
